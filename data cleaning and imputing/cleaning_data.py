...
df['sex'] = df['Sex Demographic Core'].map( data_dict['sex'] ).astype(float)

df['race'] = df['Race (Census) Race'].map ( data_dict['race'] ).astype(float)

df['region'] = df['Region Demographic'].map ( data_dict['region'] ).astype(float)

df['highest_grade'] = df["Highest Grade Completed Demographic"]

df['employment_status'] = df['Employment Status Work Demographic Core'].map ( data_dict['employment_status'] ).astype(float)

df['marital_status'] = df['Marital Status Work Demographic Core (Are you currently -- married, widowed, divorced, separated, or have you never been married?)'].map ( data_dict['marital_status'] ).astype(float)

df['religious'] = df['Religious Person Relig (To what extent do you consider yourself a religious person?)'].map( data_dict['religious']).astype(float)

df['spiritual'] = df['Spiritual Person Relig (To what extent do you consider yourself a spiritual person?)'].map( data_dict['spiritual']).astype(float)

df['party'] = df['Political Party Politics'].map( data_dict['party']).astype(float)

df['lib_cons'] = df['Liberalism-Conservatism Politics'].map( data_dict['lib_cons']).astype(float)

df['death_penalty'] = df['Death Penalty for Murderers Govt'].map( data_dict['death_penalty']).astype(float)

df['court_harsh'] = df['Court Harshness Govt (In general, do you think the courts in this area deal too harshly or not harshly enough with criminals?)'].map( data_dict['court_harsh']).astype(float)

df['income'] = df['Inflation-adjusted Family Income Demographic Work Core '].astype(float)

df['sex_partners'] = df['Sex Partners in 5 Years Sex (Now think about the past five year, and including the past 12 months, how many sex partners have you had in that five year period?)'].map( data_dict['sex_partners']).astype(float)

df['sex_freq'] = df['Sex Frequency in Last Year Sex'].map( data_dict['sex_freq']).astype(float)

df['bar'] = df['Go to Bar or Tavern Society'].map( data_dict['bar']).astype(float)

df['tv'] = df['Daily Hours Watching TV Tech']

df['relatives'] = df['Spend a Social Evening with Relatives Society'].map( data_dict['relatives']).astype(float)

df['spanking'] = df['Spanking Family (Sometimes necessary to discipline a child with a good, hard, spanking?)'].map( data_dict['spanking']).astype(float)
...
df['standard_living'] = df['Standard of Living Compared to Parents Society (Compared to your parents when they were the age you are now, do you think your own standard of living now is much better, somewhat better, about the same, somewhat worse, or much worse than theirs was?)'].map( data_dict['standard_living']).astype(float)

df['birth_control'] = df['Birth Control for Young Govt Sex (Methods of birth control should be available to teenagers between the ages of 14 and 16 if their parents do not approve)'].map( data_dict['bc_young']).astype(float)

df['immigration'] = df['Immigration Govt (Do you think the number of immigrants to America nowadays should be...)'].map( data_dict['immigration']).astype(float)
...
df['gun'] = df['Gun in Home Guns'].map( data_dict['gun']).astype(float)

df['tax_approp'] = df['Tax Appropriateness Govt (Do you consider the amount of federal income tax which you have to pay as too high, about right, or too low?)'].map( data_dict['tax_approp']).astype(float)

df['divorce_ease'] = df['Divorce Ease Govt Family (Should divorce in this country be easier or more difficult to obtain than it is now?)'].map( data_dict['divorce']).astype(float)
