...
for i in range(len(columns_ordered_by_decreasing_N)):
    ...
    column_of_var = full_columns_ordered_by_decreasing_N.index(variable) 
    # necessary because we don't want to impute the basic demographic vars - 
    # age and highest grade - so we need to start from a later column than 
    # column zero

    train_data = df.loc[pd.isnull(df[variable])==False] #training data is the 
    # data where we have an answer for the target variable
    
    train_data = train_data.ix[:,0:(column_of_var + 1)] #trimming it down to 
    # just the columns up to and including the target variable

    train_data_values = train_data.values #converting out of dataframe


    test_data = df.loc[pd.isnull(df[variable])] #test data is the data where 
    # we don't have an answer for the target variable

    test_data = test_data.ix[:,0:column_of_var] #trimming down to just the 
    # columns up to the target variable
    ...
    # minus last column, which is target variable
    ...
    # now that we have a prediction, we need to update our dataset with that 
    # prediction
    ...
    for i in range(len(ids_from_test_data)):
        df[variable][ids_from_test_data[i]] = output[i] #sets previously empty 
        # cell equal to prediction
